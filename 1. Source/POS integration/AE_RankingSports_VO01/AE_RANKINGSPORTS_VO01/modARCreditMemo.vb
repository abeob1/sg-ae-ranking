Module modARCreditMemo
    ...
    Function AR_CreditMemo(ByRef oDICompany As SAPbobsCOM.Company, _
        ...
        Try
            ...
            If oARInvoice.GetByKey(sDocEntry) Then
                ...
                Next
                oARCreditmemo.UserFields.Fields.Item("U_AB_CommEntitle").Value = oARInvoice.UserFields.Fields.Item("U_AB_CommEntitle").Value
                oARCreditmemo.UserFields.Fields.Item("U_AB_CommPercent").Value = oARInvoice.UserFields.Fields.Item("U_AB_CommPercent").Value
    ...
    Public Function AR_CreditMemo_Standalone(ByVal oDVARInvoice As DataView, ByVal oDVPayment As DataView, ByRef oCompany As SAPbobsCOM.Company, _
        ...
        Try
            ...
            oARInvoice_Doc = oCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oCreditNotes)
            ...
            oDT_Warehouse.DefaultView.RowFilter = "U_AB_POSLocCode='" & CStr(oDVARInvoice.Item(0).Row("HOutlet").ToString.Trim) & "'"
            If oDT_Warehouse.DefaultView.Count > 0 Then
                sWhsCode = oDT_Warehouse.DefaultView(0)(0).ToString().Trim()
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Warehouse Code " & sWhsCode, sFuncName)
            Else
            ...
            oARInvoice.NumAtCard = sPOSNumber ''sWhsCode & " - " & sPOSNumber
            ...
            For Each dvr As DataRowView In oDVARInvoice
                ...
                '   If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Itemcode " & dvr("DItemCode").ToString.Trim, sFuncName)
                ...
                ' If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Line Total " & dvr("DLineTotal").ToString.Trim, sFuncName)
                ...
                If Not String.IsNullOrEmpty(p_oCompDef.p_sGLAccount) Then
                    ...
                    '     If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("GL Account " & p_oCompDef.p_sGLAccount, sFuncName)
                End If

                oARInvoice.Lines.VatGroup = dvr("VatGourpSa").ToString.Trim
                '  If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Vat Group " & dvr("VatGourpSa").ToString.Trim, sFuncName)
    ...
    Function AR_Creditnote_Cancel(ByRef oDICompany As SAPbobsCOM.Company, _
                                  ByRef sCreditnote As String, ByVal dDate As Date, ByVal spostdate As String, ByRef sErrDesc As String) As Long

        Dim sFuncName As String = String.Empty
        Dim lRetCode As Long
        Dim oCreditnote As SAPbobsCOM.Documents
        Dim oCreditnoteCancellation As SAPbobsCOM.Documents
        oCreditnote = oDICompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oCreditNotes)
        ...
        Try
            ...
            sFuncName = "AR_Creditnote_Cancel"
            Console.WriteLine("Starting Function", sFuncName)
            ...
            If oCreditnote.GetByKey(sCreditnote) Then

                oCreditnoteCancellation = oCreditnote.CreateCancellationDocument()
                oCreditnoteCancellation.UserFields.Fields.Item("U_AB_Date").Value = dDate
                '' oCreditnoteCancellation.UserFields.Fields.Item("U_AB_Time").Value = dpostdate

                lRetCode = oCreditnoteCancellation.Add()

                If lRetCode <> 0 Then
                    sErrDesc = oDICompany.GetLastErrorDescription
                    Call WriteToLogFile(sErrDesc, sFuncName)
                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
                    AR_Creditnote_Cancel = RTN_ERROR
                Else
                    Console.WriteLine("Completed with SUCCESS ", sFuncName)
                    oDICompany.GetNewObjectCode(sDocEntry)
                    sSQL = "Update ORIN set [U_AB_Time] = '" & stime & "' WHERE DocEntry = '" & sDocEntry & "'"
                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Time Change (Credit Note) " & sSQL, sFuncName)
                    oRset.DoQuery(sSQL)
                    sErrDesc = String.Empty
                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with SUCCESS.", sFuncName)
                    AR_Creditnote_Cancel = RTN_SUCCESS
                    sErrDesc = String.Empty
                End If
            Else

                sErrDesc = "No matching records found in the AR Credit Memo " & sCreditnote
                Call WriteToLogFile(sErrDesc, sFuncName)
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Completed with ERROR", sFuncName)
                AR_Creditnote_Cancel = RTN_ERROR
            End If
        ...
        Finally
            ...
            System.Runtime.InteropServices.Marshal.ReleaseComObject(oCreditnoteCancellation)
            oCreditnote = Nothing
            oCreditnoteCancellation = Nothing
        End Try
    End Function
